...
void Sidebar::sync_ams_list()
{
    ...
    if (unknowns > 0) {
        MessageDialog dlg(this,
            _L("There are some unknown filaments mapped to generic preset. Please update FullSpectrum or restart FullSpectrum to check if there is an update to system presets."),
            _L("Sync filaments with AMS"), wxOK);
        dlg.ShowModal();
    }
                ...
                Semver             file_version;
                {
                    ...
                    if (en_3mf_file_type == En3mfType::From_Prusa) {
                        ...
                        if(load_type != LoadType::LoadGeometry)
                            show_info(q, _L("The 3mf is not supported by FullSpectrum, load geometry data only."), _L("Load 3mf"));
                    }
                    ...
                    //             show_info(q, _L("The 3mf is generated by old FullSpectrum, load geometry data only."), _L("Load 3mf"));
                    //         else
                    //             show_info(q, _L("The 3mf is not supported by FullSpectrum, load geometry data only."), _L("Load 3mf"));
                    //     }
                    ...
                    //     }
                    // }
                    // Orca: check if the project is created with FullSpectrum 2.2.0-alpha and use the sparse infill rotation template for non-safe infill patterns
                    else if (false) {
                        if (!config_loaded.opt_string("sparse_infill_rotate_template").empty()) {
                            ...
                            if (!is_safe_to_rotate) {
                                wxString msg_text = _(
                                    L("This project was created with an FullSpectrum and uses "
                                ...
                                msg_text += "\n\n" +
                                            _(L("Would you like FullSpectrum to automatically fix this by clearing the rotation template settings?"));
...
void Plater::priv::set_project_name(const wxString& project_name)
{
    ...
    //update topbar title
#ifdef __WINDOWS__
    wxGetApp().mainframe->SetTitle(m_project_name + " - FullSpectrum");
...
void Plater::priv::bring_instance_forward() const
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "FullSpectrum window going forward";
            ...
            http.on_progress([&percent, &cont, &msg, &filesize, &size_limit](Http::Progress progress, bool& cancel) {
                ...
                .on_error([&msg, &cont, &retry_count, max_retries](std::string body, std::string error, unsigned http_status) {
                    ...
                    if (retry_count == max_retries) {
                        msg = _L("Importing to FullSpectrum failed. Please download the file and manually import it.");
                        cont = false;
                    }
                })
...
void Plater::show_object_info()
{
    ...
    if (non_manifold_edges > 0) {
        info_manifold += into_u8("\n" + _L("Tips:") + "\n" +_L("\"Fix Model\" feature is currently only on Windows. Please repair the model on FullSpectrum(windows) or CAD softwares."));
    }
    #endif //APPLE & LINUX
