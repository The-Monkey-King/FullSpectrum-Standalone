...
function build_slicer() {
        ...
        if [ "$ARCH" == "universal" ] || [ "$ARCH" == "$_ARCH" ]; then
            ...
            if [ "1." != "$BUILD_ONLY". ]; then
                cmake "${PROJECT_DIR}" \
                    ...
                    -DCMAKE_INSTALL_PREFIX="$PWD/FullSpectrum" \
                    -DCMAKE_BUILD_TYPE="$BUILD_CONFIG" \
            ...
            if [ "$SLICER_BUILD_TARGET" = "all" ] || [ "$SLICER_BUILD_TARGET" = "ALL_BUILD" ]; then
                cmake --build . --config "$BUILD_CONFIG" --target FullSpectrum_profile_validator || echo "Warning: FullSpectrum_profile_validator build failed or not available"
            fi
        )
    ...
    (
        cd "$PROJECT_BUILD_DIR"
        mkdir -p FullSpectrum
        cd FullSpectrum
        # remove previously built app
        rm -rf "./FullSpectrum.app"
        # determine source app path (handle both space and underscore names)
        APP_SOURCE_PATH="../src$BUILD_DIR_CONFIG_SUBDIR/FullSpectrum.app"
        ...
        cp -pR "$APP_SOURCE_PATH" "./FullSpectrum.app"
        # fix resources
        resources_path=$(readlink "./FullSpectrum.app/Contents/Resources")
        rm "./FullSpectrum.app/Contents/Resources"
        cp -R "$resources_path" "./FullSpectrum.app/Contents/Resources"
        # delete .DS_Store file
        find "./FullSpectrum.app/" -name '.DS_Store' -delete
        ...
        APP_MACOS_DIR='./FullSpectrum.app/Contents/MacOS'
        APP_FRAMEWORKS_DIR='./FullSpectrum.app/Contents/Frameworks'
        EXECUTABLE="${APP_MACOS_DIR}/FullSpectrum"
        ...
        if [ -f "${LIBSENTRY}" ]; then
            ...
            # Update rpath in FullSpectrum to use @executable_path relative path
            if [ -f "${EXECUTABLE}" ]; then
                echo "Updating libsentry.dylib rpath in FullSpectrum..."
        ...
        # Copy FullSpectrum_profile_validator.app if it exists
        if [ -f "../src$BUILD_DIR_CONFIG_SUBDIR/FullSpectrum_profile_validator.app/Contents/MacOS/FullSpectrum_profile_validator" ]; then
            echo "Copying FullSpectrum_profile_validator.app..."
            rm -rf ./FullSpectrum_profile_validator.app
            cp -pR "../src$BUILD_DIR_CONFIG_SUBDIR/FullSpectrum_profile_validator.app" ./FullSpectrum_profile_validator.app
            # delete .DS_Store file
            find ./FullSpectrum_profile_validator.app/ -name '.DS_Store' -delete
        fi
        ...
        if [ -f "${APP_MACOS_DIR}/FullSpectrum" ]; then
            echo "Generating dSYM for FullSpectrum..."
            dsymutil "${APP_MACOS_DIR}/FullSpectrum" -o "${DSYM_DIR}/FullSpectrum.dSYM" 2>/dev/null || echo "Warning: Failed to generate dSYM for FullSpectrum (no debug symbols?)"
        fi
        ...
        if [ -f "./FullSpectrum_profile_validator.app/Contents/MacOS/FullSpectrum_profile_validator" ]; then
            echo "Generating dSYM for FullSpectrum_profile_validator..."
            dsymutil "./FullSpectrum_profile_validator.app/Contents/MacOS/FullSpectrum_profile_validator" -o "${DSYM_DIR}/FullSpectrum_profile_validator.dSYM" 2>/dev/null || true
        fi
    ...
    # fi
        ...
        # zip -FSr FullSpectrum${ver}_Mac_${_ARCH}.zip OrcaSlicer.app
...
function build_universal() {
    ...
    mkdir -p "$PROJECT_BUILD_DIR/FullSpectrum"
    UNIVERSAL_APP="$PROJECT_BUILD_DIR/FullSpectrum/FullSpectrum.app"
    rm -rf "$UNIVERSAL_APP"
    cp -R "$PROJECT_DIR/build/arm64/FullSpectrum/FullSpectrum.app" "$UNIVERSAL_APP"
    
    # Get the binary path inside the .app bundle
    BINARY_PATH="Contents/MacOS/FullSpectrum"
    
    # Create universal binary using lipo
    lipo -create \
        "$PROJECT_DIR/build/x86_64/FullSpectrum/FullSpectrum.app/$BINARY_PATH" \
        "$PROJECT_DIR/build/arm64/FullSpectrum/FullSpectrum.app/$BINARY_PATH" \
        -output "$UNIVERSAL_APP/$BINARY_PATH"
        
    echo "Universal binary created at $UNIVERSAL_APP"
    
    # Create universal crashpad_handler if both architectures have it
    CRASHPAD_ARM64="${PROJECT_DIR}/build/arm64/FullSpectrum/FullSpectrum.app/Contents/MacOS/crashpad_handler"
    CRASHPAD_X86="${PROJECT_DIR}/build/x86_64/FullSpectrum/FullSpectrum.app/Contents/MacOS/crashpad_handler"
    ...
    LIBSENTRY_ARM64="${PROJECT_DIR}/build/arm64/FullSpectrum/FullSpectrum.app/Contents/Frameworks/libsentry.dylib"
    LIBSENTRY_X86="${PROJECT_DIR}/build/x86_64/FullSpectrum/FullSpectrum.app/Contents/Frameworks/libsentry.dylib"
    ...
    # Update rpath in universal FullSpectrum
    if [ -f "${LIBSENTRY_UNIVERSAL}" ]; then
        echo "Updating libsentry.dylib rpath in universal FullSpectrum..."
    ...
    if [ -f "$PROJECT_DIR/build/arm64/FullSpectrum/FullSpectrum_profile_validator.app/Contents/MacOS/FullSpectrum_profile_validator" ] && \
       [ -f "$PROJECT_DIR/build/x86_64/FullSpectrum/FullSpectrum_profile_validator.app/Contents/MacOS/FullSpectrum_profile_validator" ]; then
        echo "Creating universal binary for FullSpectrum_profile_validator..."
        UNIVERSAL_VALIDATOR_APP="$PROJECT_BUILD_DIR/FullSpectrum/FullSpectrum_profile_validator.app"
        rm -rf "$UNIVERSAL_VALIDATOR_APP"
        cp -R "$PROJECT_DIR/build/arm64/FullSpectrum/FullSpectrum_profile_validator.app" "$UNIVERSAL_VALIDATOR_APP"
        
        # Get the binary path inside the profile validator .app bundle
        VALIDATOR_BINARY_PATH="Contents/MacOS/FullSpectrum_profile_validator"
        
        # Create universal binary using lipo
        lipo -create \
            "$PROJECT_DIR/build/x86_64/FullSpectrum/FullSpectrum_profile_validator.app/$VALIDATOR_BINARY_PATH" \
            "$PROJECT_DIR/build/arm64/FullSpectrum/FullSpectrum_profile_validator.app/$VALIDATOR_BINARY_PATH" \
            -output "$UNIVERSAL_VALIDATOR_APP/$VALIDATOR_BINARY_PATH"
            
        echo "Universal binary for FullSpectrum_profile_validator created at $UNIVERSAL_VALIDATOR_APP"
    fi
    ...
    DSYM_DIR="$PROJECT_BUILD_DIR/FullSpectrum/dSYM"
    mkdir -p "${DSYM_DIR}"
    
    # Generate dSYM for universal main app
    if [ -f "$UNIVERSAL_APP/$BINARY_PATH" ]; then
        echo "Generating dSYM for universal FullSpectrum..."
        dsymutil "$UNIVERSAL_APP/$BINARY_PATH" -o "${DSYM_DIR}/FullSpectrum.dSYM" 2>/dev/null || echo "Warning: Failed to generate dSYM for universal FullSpectrum"
    fi
    ...
    if [ -f "$UNIVERSAL_VALIDATOR_APP/$VALIDATOR_BINARY_PATH" ]; then
        echo "Generating dSYM for universal FullSpectrum_profile_validator..."
        dsymutil "$UNIVERSAL_VALIDATOR_APP/$VALIDATOR_BINARY_PATH" -o "${DSYM_DIR}/FullSpectrum_profile_validator.dSYM" 2>/dev/null || true
    fi
